#!/bin/bash
#
# Run one of the gaia scripts in a throwaway sandbox, so it can't touch your
...
#   - stub `sudo` and `systemctl` on PATH, so the systemd step (and the apt
#     calls install.sh makes through sudo) are visible no-ops and
#     /etc/systemd/system is never written;
#   - env -i for a clean environment (your real GAIA_DIR can't leak in).
...
# ${SANDBOX}/repo-mirror) simulates a new release for update.sh:
#     git -C "${GAIA_SANDBOX}/repo-mirror" tag 0.12.0
#
# Reusing a sandbox (e.g. install first, then test the update against it):
#     export GAIA_SANDBOX=$(mktemp -d /tmp/gaia_sandbox_XXXX)
#     ./utils/sandbox.sh install.sh
#     ./utils/sandbox.sh update.sh --dry-run
...
SANDBOX="${GAIA_SANDBOX:-$(mktemp -d /tmp/gaia_sandbox_XXXX)}"
echo "Sandbox: ${SANDBOX}   (remove with: rm -rf \"${SANDBOX}\")"
mkdir -p "${SANDBOX}/home" "${SANDBOX}/bin"

# The working-tree scripts may carry CRLF line endings (Windows/WSL checkouts);
# the deployed copies get dos2unix'd. Do the same for the entry script we run
# directly, else bash chokes on the \r. (Its siblings under utils/ are
# normalised on copy.)
...
cd "${SANDBOX}"   # install.sh installs into ${PWD}/gaia
...
build_mirror() {
    ...
    git -C "${MIRROR}" -c user.email=sandbox@gaia -c user.name=sandbox \
    ...
    # own GAIA_VERSION: move it onto the working tree, creating it if this is a
    # version that has not been released yet. Only do so when creating the
    # mirror: the `git fetch --tags` update.sh runs exits non-zero on a tag that
    # moved, which would abort the update and roll it back.
    if [[ "${created}" == true ]]; then
        ...
        version=$(sed -n 's/^readonly GAIA_VERSION="\([^"]*\)".*/\1/p' "${MIRROR}/scripts/install.sh")
        if [[ -n "${version}" ]]; then
...
# that update.sh --unsafe sees the latest state of the working tree.
...
if [[ -n "${REPO}" ]] &&
    ...
    build_mirror
    SANDBOX_ENV+=(GAIA_REPO="${MIRROR}")
fi

# install.sh refuses to run when GAIA_DIR is set; every other script needs it
# pointed at the sandbox install.
if [[ "$(basename "${CMD[0]}")" == install.sh ]]; then
    env -i "${SANDBOX_ENV[@]}" bash "${CMD[@]}"
else
    env -i "${SANDBOX_ENV[@]}" GAIA_DIR="${SANDBOX}/gaia" bash "${CMD[@]}"
fi
